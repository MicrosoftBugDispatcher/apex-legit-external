...
inline double normal_cdf(double x) {
    return 0.5 * (1.0 + std::erf(x / detail_numbers::SQRT2));
}
...
inline double lognormal_pdf(double t, double t0, double mu, double sigma) {
    ...
    return 1.0 / (sigma * std::sqrt(2.0 * detail_numbers::PI) * dt)
         * std::exp(-0.5 * z * z);
}
    ...
    const config& cfg = {}, uint64_t seed = 0)
{
    ...
    double dx = target_x, dy = target_y;
    double distance = std::hypot(dx, dy);
    double direction = std::atan2(dy, dx);

    if (distance < 1.0)
        return {{0.0, 0.0, 0.0}, {target_x, target_y, 50.0}};
    ...
    double tph_x = uniform(0.0, 2.0 * detail_numbers::PI);
    double tph_y = uniform(0.0, 2.0 * detail_numbers::PI);
    ...
    for (size_t i = 0; i < times.size(); ++i) {
        ...
        double bx = tx * primary_D * s;
        double by = ty * primary_D * s;
        ...
        double tr_x = tremor_amp * trem_mod
            * std::sin(2.0 * detail_numbers::PI * tremor_freq * t_s + tph_x);
        double tr_y = tremor_amp * trem_mod
            * std::sin(2.0 * detail_numbers::PI * tremor_freq * t_s + tph_y);
...
inline std::vector<aim_point> generate_aim(
    double target_x, double target_y,
    const config& cfg = {}, uint64_t seed = 0)
{
    auto path = generate(target_x, target_y, cfg, seed);
    std::vector<aim_point> aim;
    aim.reserve(path.size());

    double prev_x = 0.0, prev_y = 0.0;
    for (size_t i = 0; i < path.size(); ++i) {
        double dx = path[i].x - prev_x;
        double dy = path[i].y - prev_y;
        aim.push_back({dx, dy, path[i].t});
        prev_x = path[i].x;
        prev_y = path[i].y;
    }
    return aim;
...
inline std::vector<INPUT> build_raw_input(const std::vector<aim_point>& aim_path) {
    std::vector<INPUT> events;
    events.reserve(aim_path.size() * 2);

    for (size_t i = 0; i < aim_path.size(); ++i) {
        int dx = static_cast<int>(std::round(aim_path[i].dx));
        int dy = static_cast<int>(std::round(aim_path[i].dy));
        ...
        if (i + 1 < aim_path.size()) {
            double wait_ms = aim_path[i + 1].t - aim_path[i].t;
            if (wait_ms > 0) {
                INPUT inp = {};
                inp.type = INPUT_MOUSE;
    ...
    double straight_dist)
{
    ...
    m.endpoint_error = std::hypot(path.back().x - straight_dist, path.back().y);
    double id = std::log2(straight_dist / 40.0 + 1.0);
